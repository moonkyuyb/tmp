...
import { YellowViewBorder, GreyViewBorder, ViewBorderY, YellowLabel, BorderYInnerBox, Btn02Box, YTextInput, UnitTit,
	...
	ContractInfoHeader, InfoAert, ContractBasicsWrap } from '../../styled/provContractStyle';
...
const ProvContractPayment2Screen = ({
	...
	const onInvalid = (err) => {
		console.log(`⏬매매계약서 VALIDATON FAIL`);
		console.log(err);
		const errList = [
	...
	useEffect(()=>{
		if(salesDetail){
			if(salesDetail[0]?.s_trading_price)	setValue('pc_trading_price',salesDetail[0].s_trading_price.toString());
	...
	return (<>
		<Common.Container>
			...
			<Common.ScrollContainer20B>
				...
				<Controller
					control={control} name={'pc_trading_price'} defaultValue={'100000'}
				...
				<Controller
					control={control} name={'pc_mortgage'}
					render={({field})=>(<>
						<ViewBorderY>
							<YellowLabel><Common.TextLight12>금액</Common.TextLight12></YellowLabel>
							<BorderYInnerBox>
				...
				/>
				<Controller
					control={control} name={'pc_mortgage_bank'}
					render={({field})=>(<>
						<ViewBorderY>
							<YellowLabel><Common.TextLight12>승계은행</Common.TextLight12></YellowLabel>
							<BorderYInnerBox>
								<ArrowIcon/>
								<RNPickerSelect
				...
				<Controller
					control={control} name={'pc_middlepayment'}
				...
				/>
				<Controller
					control={control} name={'pc_middlepayment_2'}
					rules={{required:{value:middlePayment2Required, message:'2차 중도금을 입력해주세요'}}}
					render={({field})=>(<>
						<Common.SubTitle>2차 중도금</Common.SubTitle>
						<ViewBorderY>
							<YellowLabel><Common.TextLight12>금액</Common.TextLight12></YellowLabel>
							<BorderYInnerBox>
								<YTextInput placeholder={'금액 입력'} value={field.value} onChangeText={value=>{
				...
				/>
				<Controller
					control={control} name={'pc_middlepayment_datetime_2'} defaultValue='2021-10-10'
					rules={{required:{value:middlePayment2Required, message:'2차 중도금 지급일을 선택해주세요'}}}
					render={({field})=>(<>
						<ViewBorderY>
							<YellowLabel><Common.TextLight12>지불일</Common.TextLight12></YellowLabel>
							<TouchableOpacity style={{flex:1}} disabled={!middlePayment2Required} onPress={()=>{
								setTargetValue('pc_middlepayment_datetime')
								setShowDatePicker(true)
							}}>
								<BorderYInnerBox>
									<YTextInput placeholder={'지불일 선택'} editable={false} value={middlePayment2Required?field.value:''} onChangeText={field.onChange}/><DatePickerIcon/>
								</BorderYInnerBox>
							</TouchableOpacity>
						</ViewBorderY>
					</>)}
				...
				<Controller
					...
					rules={{required:{value:salesDetail[0]?.s_price_type=='sales'?true:false, message:'인도일을 선택해주세요'}}}
					render={({field})=>(<>
						<ViewBorderY>
							<YellowLabel><Common.TextLight12>인도일</Common.TextLight12></YellowLabel>
							<TouchableOpacity style={{flex:1}} onPress={()=>{
								setTargetValue('pc_contract_start')
								setShowDatePicker(true)
							}}>
								<BorderYInnerBox>
									<YTextInput placeholder={'인도일 선택'} editable={false} value={field.value} onChangeText={field.onChange}/><DatePickerIcon/>
								</BorderYInnerBox>
							</TouchableOpacity>
						</ViewBorderY>
					</>)}
...
export default ProvContractPayment2Screen